...
from torchtitan.optimizer import build_optimizers
...
def main(job_config: JobConfig):
    ...
    parallel_dims = ParallelDims(
        dp=2, # job_config.training.data_parallel_degree,
    ...
    model_config.vocab_size = 2256 # tokenizer.n_words
    ...
    # ]
    whole_model = parallelize_llama(whole_model, world_mesh, parallel_dims, job_config)

    # init_device = "cpu" if job_config.checkpoint.create_seed_checkpoint else "cuda"
    # for model in model_parts:
    whole_model.to_empty(device="cuda")
    ...
    # optimizers = build_optimizers([whole_model], job_config)
    ...
    if True:
        while train_state.step < job_config.training.steps:
            ...
            input_ids = torch.ones((8, 2048)).long()
            # labels = labels.cuda()
